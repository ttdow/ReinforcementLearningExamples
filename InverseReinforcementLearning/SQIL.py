...
for epoch in count():
    ...
    for time_steps in range(200):
        ...
        if online_memory_replay.size() > 1280:
            ...
            online_batch = online_memory_replay.sample(batch_size // 2, False)
            ...
            expert_batch = expert_memory_replay.sample(batch_size // 2, False)
            ...
            expert_batch_reward = torch.FloatTensor(expert_batch_reward).to(device)
            expert_batch_done = torch.FloatTensor(expert_batch_done).to(device)
            ...
            loss = F.mse.loss(onlineQNetwork(batch_state).gather(1, batch_action.long()), y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
