...
class DQN(nn.Module):
    ...
    # Called with either 1 element to determine next action or a batch
...
def select_action(state):
    ...
    if sample > epsilon_threshold:
        with torch.no_grad():
            ...
            # second column on max results is index of where max element was
...
def optimize_model():
    ...
    # Expected values of actions for non_final_next_states are computed based on the "older" target_net; selecting their
    #   best reward with max(1)[0]. This is merged based on the mask, such that we'll have either the expected state
    #   value or 0 in case the state was final.
